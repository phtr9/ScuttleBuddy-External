...
class LeagueReader:
    ...
    def wrapper_root_key(func):
        def wrapper(self, champion: PlayerEntity):
            if not self.champs_data:
                self.get_game_data()
            return func(self, champion=champion,
                        data=self.champs_data[champion.name.lower()][DATA_ROOT_KEY.format(champion=champion.name)])
    ...
    def get_game_data(self) -> dict:
        ...
        champions_name = [champion['championName'].lower() for champion in
                          requests.get(GAME_DATA_ENDPOINT, verify=False).json()['allPlayers']]
        ...
        self.champs_data = {name: response for name in champions_name for response in responses for key in
                            response.keys() if name in key.lower()}
        return self.champs_data
